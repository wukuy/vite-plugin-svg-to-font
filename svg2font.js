...
const svgPathify = require('svg_pathify')

let unicodeNum = 60000
        ...
        reverse: function(inputPath, outputPath) {
            try {
                mkdirSync(outputPath, () => {
                    const glyphs = fontCarrier.transfer(inputPath).allGlyph()
                    const svgs = []
                    for(key in glyphs) {
                        if(key == '&#x78;') {
                            continue
                        }
                        const svg = glyphs[key].toSvg()
                        const glyphName = glyphs[key].options.name || glyphs[key].options.glyphName || key.slice(1)
                        svgs.push([
                            '<li class="dib">',
        ...
        generate: function(inputPath, outputPath, options) {
            ...
            try {
                ...
                var travel = function(dir) {
                    const isOutputPath = RegExp(`^${outputPath}`).test(dir)
                    if(isOutputPath) return
